...
pub const FractalLevel = struct {
    ...
    pub fn deinit(self: *Self) void {
        ...
        var i: usize = 0;
        while (i < self.child_levels.items.len) : (i += 1) {
            var child = self.child_levels.items[i];
            child.deinit();
        ...
        i = 0;
        while (i < self.allocated_keys.items.len) : (i += 1) {
            self.allocator.free(self.allocated_keys.items[i]);
        }
    ...
    pub fn addNode(self: *Self, node: FractalNodeData) !void {
        ...
        try self.allocated_keys.append(id_copy);
    ...
    pub fn getEdgesConst(self: *const Self, source_id: []const u8) []FractalEdgeData {
    ...
    pub fn addEdge(self: *Self, edge: FractalEdgeData) !void {
        const source_copy = try self.allocator.dupe(u8, edge.source_id);
        errdefer self.allocator.free(source_copy);

        var result = try self.edges.getOrPut(source_copy);
        if (!result.found_existing) {
            result.value_ptr.* = ArrayList(FractalEdgeData).init(self.allocator);
    ...
    pub fn removeEdge(self: *Self, source_id: []const u8, target_id: []const u8) bool {
        if (self.edges.getPtr(source_id)) |edge_list| {
            ...
            while (i < edge_list.items.len) {
                if (std.mem.eql(u8, edge_list.items[i].target_id, target_id)) {
    ...
    pub fn computeLocalFractalDimension(self: *Self) f64 {
        ...
        var i: usize = 0;
        while (i < box_sizes.len) : (i += 1) {
            const size = box_sizes[i];
    ...
    fn estimateBoxCount(self: *Self, box_size: usize) usize {
        ...
        const node_cnt = self.node_count;
        if (node_cnt == 0) return 0;

        const box_size_f = @as(f64, @floatFromInt(box_size));
        const estimated = @as(usize, @intFromFloat(@ceil(@as(f64, @floatFromInt(node_cnt)) / box_size_f)));
    ...
    pub fn getDepth(self: *const Self) usize {
        ...
        var i: usize = 0;
        while (i < self.child_levels.items.len) : (i += 1) {
            const child_depth = self.child_levels.items[i].getDepth();
    ...
    pub fn getTotalNodeCount(self: *const Self) usize {
        ...
        var i: usize = 0;
        while (i < self.child_levels.items.len) : (i += 1) {
            total += self.child_levels.items[i].getTotalNodeCount();
        }
        return total;
    }
};
...
pub const FractalTree = struct {
    ...
    pub fn init(allocator: Allocator, max_depth: usize, branching_factor: usize) !Self {
        ...
        hasher.update(std.mem.asBytes(&branching_factor));
        const hash_result = hasher.finalResult();
        @memcpy(&tree_id, &hash_result);
    ...
    fn searchLevel(self: *Self, level: *FractalLevel, node_id: []const u8) ?*FractalNodeData {
        ...
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            if (self.searchLevel(level.child_levels.items[i], node_id)) |found| {
                return found;
            }
        }
    ...
    fn searchLevelConst(self: *const Self, level: *FractalLevel, node_id: []const u8) ?FractalNodeData {
        ...
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            if (self.searchLevelConst(level.child_levels.items[i], node_id)) |found| {
                return found;
            }
        }
    ...
    fn deleteFromLevel(self: *Self, level: *FractalLevel, node_id: []const u8) bool {
        ...
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            if (self.deleteFromLevel(level.child_levels.items[i], node_id)) {
                return true;
            }
        }
    ...
    fn traversePreOrder(self: *Self, level: *FractalLevel, depth: usize, callback: TraversalCallback) void {
        ...
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            self.traversePreOrder(level.child_levels.items[i], depth + 1, callback);
        }
    }
    ...
    fn traversePostOrder(self: *Self, level: *FractalLevel, depth: usize, callback: TraversalCallback) void {
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            self.traversePostOrder(level.child_levels.items[i], depth + 1, callback);
        }
        callback(level, depth);
    }

    fn traverseLevelOrder(self: *Self, callback: TraversalCallback) void {
        var queue = ArrayList(struct { level: *FractalLevel, depth: usize }).init(self.allocator);
        defer queue.deinit();

        queue.append(.{ .level = self.root, .depth = 0 }) catch return;
        ...
        while (queue.items.len > 0) {
            ...
            var i: usize = 0;
            while (i < item.level.child_levels.items.len) : (i += 1) {
                queue.append(.{ .level = item.level.child_levels.items[i], .depth = item.depth + 1 }) catch continue;
            }
        }
    }
    ...
    fn collectAllNodes(self: *Self, level: *FractalLevel, nodes: *ArrayList(FractalNodeData)) !void {
        ...
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            try self.collectAllNodes(level.child_levels.items[i], nodes);
        }
    }
    ...
    fn computeLevelDimension(self: *Self, level: *FractalLevel) f64 {
        ...
        var i: usize = 0;
        while (i < level.child_levels.items.len) : (i += 1) {
            child_dim_sum += self.computeLevelDimension(level.child_levels.items[i]);
        }
...
pub const SelfSimilarIndex = struct {
    ...
    pub fn deinit(self: *Self) void {
        ...
        while (iter.next()) |entry| {
            ...
            var i: usize = 0;
            while (i < locations.items.len) : (i += 1) {
                locations.items[i].deinit();
            }
            locations.deinit();
        }
        ...
        var i: usize = 0;
        while (i < self.pattern_keys.items.len) : (i += 1) {
            self.allocator.free(self.pattern_keys.items[i]);
        }
    ...
    pub fn addPattern(self: *Self, pattern: []const u8, location: PatternLocation) !void {
        ...
        var result = try self.patterns.getOrPut(pattern_copy);
        if (!result.found_existing) {
            result.value_ptr.* = ArrayList(PatternLocation).init(self.allocator);
        ...
        try result.value_ptr.append(location);
...
pub fn CoalescedHashMap(comptime K: type, comptime V: type) type {
    return struct {
        ...
        fn hash(self: *const Self, key: K) usize {
            _ = self;
            if (comptime @TypeOf(key) == []const u8) {
                var hasher = std.hash.Wyhash.init(0);
                hasher.update(key);
                return @as(usize, @intCast(hasher.final()));
            } else if (comptime @typeInfo(K) == .Int) {
                return @as(usize, @intCast(@as(u64, @bitCast(key)) % std.math.maxInt(usize)));
            } else {
                var hasher = std.hash.Wyhash.init(0);
...
pub const LRUCacheEntry = struct {
    ...
    pub fn init(allocator: Allocator, key: []const u8, value: []const u8) !Self {
...
pub const LRUCache = struct {
    entries: StringHashMap(LRUCacheEntry),
    access_order: ArrayList(usize),
    key_to_order_idx: StringHashMap(usize),
    order_idx_to_key: ArrayList([]const u8),
    capacity: usize,
    current_size: usize,
    max_memory: usize,
    ...
    pub fn init(allocator: Allocator, capacity: usize, max_memory: usize) Self {
        return Self{
            .entries = StringHashMap(LRUCacheEntry).init(allocator),
            .access_order = ArrayList(usize).init(allocator),
            .key_to_order_idx = StringHashMap(usize).init(allocator),
            .order_idx_to_key = ArrayList([]const u8).init(allocator),
            .capacity = capacity,
            .current_size = 0,
    ...
    pub fn deinit(self: *Self) void {
        ...
        self.key_to_order_idx.deinit();
        self.order_idx_to_key.deinit();
    }
    ...
    pub fn put(self: *Self, key: []const u8, value: []const u8) !void {
        ...
        while (self.current_size >= self.capacity or
               (self.current_memory + entry_size > self.max_memory and self.current_size > 0)) {
            if (!self.evictLRU()) {
                break;
            }
        }
        ...
        if (self.entries.getPtr(key)) |existing| {
            ...
            existing.deinit();
            var new_entry = try LRUCacheEntry.init(self.allocator, key, value);
            existing.* = new_entry;
            self.current_memory += new_entry.size;
            self.moveToFront(key);
        } else {
            var new_entry = try LRUCacheEntry.init(self.allocator, key, value);
            ...
            try self.entries.put(key_copy, new_entry);

            const order_idx = self.order_idx_to_key.items.len;
            try self.order_idx_to_key.append(key_copy);
            try self.key_to_order_idx.put(key_copy, order_idx);
            try self.access_order.append(order_idx);
    ...
    fn moveToFront(self: *Self, key: []const u8) void {
        if (self.key_to_order_idx.get(key)) |order_idx| {
            var i: usize = 0;
            while (i < self.access_order.items.len) {
                if (self.access_order.items[i] == order_idx) {
                    const removed = self.access_order.orderedRemove(i);
                    self.access_order.append(removed) catch return;
                    return;
                }
                i += 1;
            }
    ...
    fn evictLRU(self: *Self) bool {
        ...
        const lru_order_idx = self.access_order.orderedRemove(0);
        if (lru_order_idx >= self.order_idx_to_key.items.len) {
            return false;
        }

        const lru_key = self.order_idx_to_key.items[lru_order_idx];
        ...
        if (self.entries.fetchRemove(lru_key)) |removed| {
            ...
            self.evictions += 1;
            entry.deinit();
            _ = self.key_to_order_idx.remove(lru_key);
            self.allocator.free(removed.key);
            return true;
        }
        return false;
    ...
    pub fn remove(self: *Self, key: []const u8) bool {
        if (self.entries.fetchRemove(key)) |removed| {
            ...
            if (self.key_to_order_idx.get(key)) |order_idx| {
                ...
                _ = self.key_to_order_idx.remove(key);
            }
            self.allocator.free(removed.key);
            return true;
        }
    ...
    pub fn clear(self: *Self) void {
        ...
        self.key_to_order_idx.clearRetainingCapacity();
        self.order_idx_to_key.clearRetainingCapacity();
...
pub const FNDSManager = struct {
    ...
    pub fn deinit(self: *Self) void {
        ...
        while (i < self.index_keys.items.len) : (i += 1) {
            self.allocator.free(self.index_keys.items[i]);
        }
