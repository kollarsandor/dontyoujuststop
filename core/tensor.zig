...
const Shape = struct {
    ...
    pub fn init(allocator: Allocator, shape: []const usize) !Shape {
        const n = shape.len;
        var dims = try allocator.alloc(usize, n);
        var strides = try allocator.alloc(usize, n);
        @memcpy(dims, shape);
        if (n > 0) {
            strides[n - 1] = 1;
            var i: usize = n - 1;
...
pub const Tensor = struct {
    ...
    pub fn init(allocator: Allocator, shape: []const usize) !Tensor {
        ...
        cow.* = false;
        return .{ .data = data, .shape = sh, .allocator = allocator, .refcount = refcount, .cow = cow };
    }
    ...
    fn ensureWritable(self: *Tensor) !void {
        const is_cow = @atomicLoad(bool, self.cow, .Acquire);
        if (is_cow) {
            const new_data = try self.allocator.alloc(f32, self.data.len);
            @memcpy(new_data, self.data);
            self.data = new_data;
            @atomicStore(bool, self.cow, false, .Release);
        }
    }

    fn markShared(self: *Tensor) void {
        @atomicStore(bool, self.cow, true, .Release);
    }

    pub fn newView(self: *Tensor, shape: Shape) !Tensor {
        if (shape.totalSize() != self.shape.totalSize()) return Error.InvalidShape;
        self.retain();
        const cow = try self.allocator.create(bool);
        cow.* = @atomicLoad(bool, self.cow, .Acquire);
        return .{ .data = self.data, .shape = shape, .allocator = self.allocator, .refcount = self.refcount, .cow = cow };
    }
    ...
    pub fn reshape(self: *Tensor, new_shape: []const usize) !void {
        ...
        if (total != self.shape.totalSize()) return Error.InvalidShape;
    ...
    pub fn view(self: *Tensor, new_shape: []const usize) !Tensor {
        ...
        const cow = try self.allocator.create(bool);
        cow.* = true;
        return .{ .data = self.data, .shape = new_sh, .allocator = self.allocator, .refcount = self.refcount, .cow = cow };
    }
    ...
    pub fn slice(self: *Tensor, starts: []const usize, ends: []const usize) !Tensor {
        ...
        const cow = try self.allocator.create(bool);
        cow.* = true;
        return .{ .data = new_data, .shape = new_sh, .allocator = self.allocator, .refcount = self.refcount, .cow = cow };
    }
    ...
    pub fn max(self: *const Tensor, allocator: Allocator, axis: usize) !Tensor {
        ...
        const new_size = result.shape.totalSize();
        var out_idx: usize = 0;
    ...
    pub fn mean(self: *const Tensor, allocator: Allocator, axis: usize) !Tensor {
        ...
        summed.divScalar(@as(f32, @floatFromInt(self.shape.dims[axis])));
        return summed;
    }
    ...
    pub fn stack(allocator: Allocator, tensors: []const Tensor, axis: usize) !Tensor {
        ...
        const new_t = try init(allocator, new_shape);
        var reduced_shape = try allocator.alloc(usize, ndim);
        defer allocator.free(reduced_shape);
    ...
    fn getFromFlat(self: *const Tensor, flat: usize, axis_idx: usize, reduced_shape: []const usize) !f32 {
        var in_indices = try self.allocator.alloc(usize, self.shape.dims.len);
        defer self.allocator.free(in_indices);
        @memset(in_indices, 0);
        var rem = flat;
        var j: usize = 0;
        for (reduced_shape) |dim| {
            if (rem >= dim) return Error.OutOfBounds;
            in_indices[j] = rem;
            rem = 0;
        }
    ...
    pub fn isClose(self: *const Tensor, other: *const Tensor, rtol: f32, atol: f32) !bool {
        ...
        while (i < self.data.len) : (i += 1) {
            const diff = math.fabs(self.data[i] - other.data[i]);
            if (diff > atol + rtol * math.fabs(other.data[i])) return false;
        }
        return true;
    }
    ...
    pub fn spectralNorm(self: *const Tensor, allocator: Allocator, max_iter: u32, tol: f32) !f32 {
        ...
        while (iter < max_iter) : (iter += 1) {
            ...
            if (math.fabs(radius - last_radius) < tol) return math.fabs(radius);
            last_radius = radius;
        }
        return math.fabs(last_radius);
    }
    ...
    pub fn inverse(self: *const Tensor, allocator: Allocator) !Tensor {
        ...
        while (i < n) : (i += 1) {
            ...
            var j: usize = i + 1;
            while (j < n) : (j += 1) {
                if (math.fabs(mat.data[j * n + i]) > math.fabs(mat.data[pivot * n + i])) {
                    pivot = j;
                }
            }
            if (math.fabs(mat.data[pivot * n + i]) < 1e-10) return Error.SingularMatrix;
            if (pivot != i) {
                var k: usize = 0;
                while (k < n) : (k += 1) {
    ...
    pub fn eig(self: *const Tensor, allocator: Allocator) !struct { vals: Tensor, vecs: Tensor } {
        ...
        while (iter < 100) : (iter += 1) {
            var qr_result = try mat.qr(allocator);
            var new_mat = try matmul(&qr_result.r, &qr_result.q, allocator);
            mat.deinit();
            mat = new_mat;
            var new_vecs = try matmul(&vecs, &qr_result.q, allocator);
            vecs.deinit();
            vecs = new_vecs;
            qr_result.q.deinit();
            qr_result.r.deinit();
            ...
            var ii: usize = 1;
            while (ii < n) : (ii += 1) {
                if (math.fabs(mat.data[ii * n + (ii - 1)]) > 1e-10) {
                    converged = false;
                    break;
                }
            }
    ...
    pub fn det(self: *const Tensor, allocator: Allocator) !f32 {
        ...
        while (i < n) : (i += 1) {
            ...
            if (math.fabs(mat.data[pivot * n + i]) < 1e-10) return 0.0;
            if (pivot != i) {
                var k: usize = 0;
                while (k < n) : (k += 1) {
    ...
    pub fn norm(self: *const Tensor, order: f32) f32 {
        var total: f32 = 0.0;
        for (self.data) |val| {
            total += math.pow(f32, math.fabs(val), order);
        }
    ...
    pub fn save(self: *const Tensor, writer: anytype) !void {
        try writer.writeInt(usize, self.shape.dims.len, .Little);
